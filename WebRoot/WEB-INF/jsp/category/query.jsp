    ...
    <script type="text/javascript">  
        ...
        $(function(){  
            $('#dg').datagrid({     
                ...
                url:'${pageContext.request.contextPath}/category/query.html',  
			    loadMsg:'Loading......',
				...
				toolbar: [{
					...
					handler: function(){
						parent.$("#win").window({
							...
							width:350,
							height:200,
							content:'<iframe src="${pageContext.request.contextPath}/category/tosave.html" frameborder="0" width="100%" height="100%"/>'
						});
					}
				},'-',{
					iconCls: 'icon-edit',
					...
					handler: function(){
						...
						if(rows.length == 0) {
							...
							parent.$("#win").window({
								...
								width:350,
								height:250,
								content:'<iframe src="${pageContext.request.contextPath}/category/toupdate.html" frameborder="0" width="100%" height="100%"/>'
							});
							//2. 
						}
					}
						...
						if(rows.length == 0) {
							...
							$.messager.confirm('删除的确认对话框', '您确定要删除此项吗？', function(r){
								if (r){
								    ...
								    //2. 发送ajax请求
								    $.post("${pageContext.request.contextPath}/category/deleteByIds.html",{ids:ids},function(result){
								    	if(result == "true") {
