...
<html>  
    ...
    <script type="text/javascript">  
        $(function(){  
            ...
            $("#cc").combobox({     
                ...
                url:'${pageContext.request.contextPath}/category/queryCategory.html',    
                valueField:'id',      
            ...
            $("#ff").form('load',{  
                ...
                open:rows[0].open,  
                cid:rows[0].cid //EasyUI不支持account.id这种点操作，所以要加个引号  
            });  
            ...
            $("#btn").click(function(){  
                ...
                if($("#ff").form("validate")) {  
                    //调用submit方法提交数据  
                    $("#ff").form('submit', {  
                        url: '${pageContext.request.contextPath}/product/update.html', //提交时将请求传给productAction的update方法执行  
                        success: function(){  
                            //如果成功了，关闭当前窗口，并刷新页面  
  ...
  <body>  
    <form title="更新商品" id="ff" method="post" enctype="multipart/form-data">  
        ...
        <div>  
            <label>更新图片:</label> <input type="file" name="fileImage" />  
        </div>  
        <div>     
            <label for="account">所属商品类:</label>  
             <!-- 远程加载管理员数据 -->  
             <input id="cc" name="cid" />  
        </div>  
        <div>  
